...
import com.bitsinharmony.recognito.converters.PrimitiveTypeConverter;
...
/**
 * Utility methods for loading voice samples from files
 * 
 * @author Amaury Crickx
 */
public abstract class JavaXSoundFileHelper implements RecognitoSoundFileConverter {

    public static double[] readAudioInputStream(AudioInputStream is) 
            throws IOException, RecognitoUnsupportedAudioFileException  {
        ...
        if (!originalFormat.matches(format)) {
            if (AudioSystem.isConversionSupported(format, originalFormat)) {
                ...
                throw new RecognitoUnsupportedAudioFileException("Alas, the system could not decode your file type." +
        ...
        while ((bytesRead = localIs.read(buffer)) > -1) {
            ...
            for (int i = 0; i < wordCount; i++) {
                double d = (double) PrimitiveTypeConverter.byteArrayToShort(
                        buffer, 2 * i, format.isBigEndian()) / 32768;
